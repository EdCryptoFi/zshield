...
export default function DashboardOverlay() {
  ...
  const chartBars = useMemo(() => Array.from({ length: 36 }, (_, i) => {
    const v = Math.sin(i / 5) * 0.3 + 0.5 + (i % 7 === 0 ? 0.25 : 0)
  ...
  return (
    <div style={{ display: 'grid', gap: 14 }}>
      ...
      <div style={{ border: '1px solid var(--line)', padding: '14px 18px', display: 'grid', gridTemplateColumns: '1fr auto', gap: 16, alignItems: 'center' }}>
        <div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--ink-mute)', letterSpacing: '0.24em', textTransform: 'uppercase', marginBottom: 5 }}>
            verified identity
          </div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--gold-bright)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, alignItems: 'start' }}>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          <div>
            ...
            <div style={{ border: '1px solid var(--line)', overflow: 'hidden' }}>
              {PROVEN.map((p, i) => (
                <div key={i} style={{ padding: '9px 12px', borderBottom: i < PROVEN.length - 1 ? '1px solid var(--line-soft)' : undefined, display: 'flex', gap: 8, alignItems: 'flex-start' }}>
          ...
          <div>
            ...
            <div style={{ border: '1px solid var(--line)', overflow: 'hidden' }}>
              {HIDDEN.map((h, i) => (
                <div key={i} style={{ padding: '8px 12px', borderBottom: i < HIDDEN.length - 1 ? '1px solid var(--line-soft)' : undefined, display: 'flex', gap: 8, alignItems: 'center' }}>
                  <span style={{ color: 'var(--ink-mute)', fontSize: 11, flexShrink: 0 }}>✗</span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--ink-mute)' }}>{h}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
            ...
            <div style={{ border: '1px solid var(--line)', overflow: 'hidden' }}>
              {FLOW.map((f, i) => (
                <div key={i} style={{
                  ...
                  <span style={{ padding: '8px 10px', fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--gold)', fontWeight: 700, borderRight: '1px solid var(--line-soft)' }}>{f.n}</span>
                  <div style={{ padding: '7px 12px' }}>
                    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--ink)', fontWeight: 500 }}>{f.step}</div>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${chartBars.length}, 1fr)`, gap: 2, alignItems: 'end', height: 50 }}>
              {chartBars.map((b, i) => (
                <div key={i} style={{
                  height: `${b * 100}%`,
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 5 }}>
              {['00:00', '12:00', 'now'].map(t => (
                <span key={t} style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--ink-mute)' }}>{t}</span>
              ))}
            </div>
          </div>

          <div>
            <SectionHeader label="did document" />
            <details style={{ border: '1px solid var(--line)' }}>
              ...
              <pre style={{
                ...
                overflow: 'auto', maxHeight: 160, background: 'rgba(0,0,0,0.2)',
              }}>{didDoc}</pre>
            </details>
          </div>
        </div>
